...
#detect competing outgoing transfers for 10 largest flows per DTN 
	...
	for dtn in globus_nodes:
		for file in sys.argv[1]:
				with open(file,'r') as f:
					...
					final_directory = dtn + f.name +'_dir'
					if not os.path.exists(final_directory):
   						os.makedirs(final_directory)
					tstat_data = [json.loads(line) for line in f]
					print(dtn)
					dtnlist = [x for x in tstat_data if x['_source']['source'].startswith(dtn) and '.nersc.gov' not in x['_source']['dest']]
					...
					for i in dtnlist_sorted[:10]:
						...
						print('one entry')
						with open(os.path.join(final_directory,i['_source']['dest']+'_out.txt') ,'a+') as fd:
							fd.write('{0} {1} {2} {3} {4} {5}\n'.format(i['_source']['dest'], float(i['_source']['values']['file_size_MB']),i['_source']['values']['throughput_Mbps'],start_time, end_time,100* (float(i['_source']['values']['tcp_rexmit_bytes'])/float(i['_source']['values']['num_bytes'])) ))
						#time.sleep(1)
							for x in dtnlist_sorted[11:]:
								...
								if (datetime.datetime.fromtimestamp(int(x['_source']["end"])/1000) < (start_time + duration) and datetime.datetime.fromtimestamp(int(x['_source']["start"])/1000) > start_time):
									...
									print(x['_source']['dest'], x['_source']['values']['throughput_Mbps'],datetime.datetime.fromtimestamp(int(x['_source']["start"])/1000), datetime.datetime.fromtimestamp(int(x['_source']["end"])/1000),100* (float(x['_source']['values']['tcp_rexmit_bytes'])/float(x['_source']['values']['num_bytes'])))
									fd.write('{0} {1} {2} {3} {4} {5}\n'.format(x['_source']['dest'], float(x['_source']['values']['file_size_MB']),x['_source']['values']['throughput_Mbps'],datetime.datetime.fromtimestamp(int(x['_source']["start"])/1000), datetime.datetime.fromtimestamp(int(x['_source']["end"])/1000),100* (float(x['_source']['values']['tcp_rexmit_bytes'])/float(x['_source']['values']['num_bytes']))))
